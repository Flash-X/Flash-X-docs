...
The ``Gravity`` unit supplied with |flashx| computes gravitational source
terms for the code. These source terms can take the form of the
gravitational potential :math:`\phi({\bf x})` or the gravitational
acceleration :math:`{\bf g}({\bf x})`,
...
``Gravity`` implementation, no additional storage in ``unk`` for holding
...
may be ignored. The ``PlanePar`` field is cheaper to compute than the
``PointMass`` field described below, since no fractional powers of the
...
This ``PointMass`` implementation describes the gravitational field due
...
The ``UserDefined`` implementation is a placeholder module for the user
...
``PPDEFINE |flashx|_GRAVITY_TIMEDEP`` in your setup’s Config file.
...
The self-consistent gravity algorithm supplied with |flashx| computes the
...
``Gravity/GravityMain/Multipole`` and ``Gravity/GravityMain/Multigrid``.
The initialization routines for these algorithms are contained in the
``Gravity`` unit, but the actual implementations are contained below the
``Grid`` unit due to code architecture constraints.
...
For example, the PPM algorithm supplied with |flashx| uses the following
...
:math:`\phi_i^{n-1}` and :math:`\phi_i^n`. The :math:`{\tt Poisson}`
gravity implementation supplies a mesh variable to contain the potential
from the previous timestep; future releases of |flashx| may permit the
...
The ``Tree`` implementation of the gravity unit in
``physics/Gravity/GravityMain/Poisson/BHTree`` is meant to be used
together with the tree solver implementation
``Grid/GridSolvers/BHTree/Wunsch``. It either calculates the
gravitational potential field which is subsequently differentiated in
subroutine ``physics/Gravity/Gravity_accelOneRow`` to obtain the
...
specifying ``bhtreeAcc=1`` as a command line argument of the setup
script.
...
most reasonable choice of :math:`m` seems to be :math:`m=2`. This MAC is
...
``Gravity/grv_bhAccErr`` and ``Gravity/grv_bhUseRelAccErr`` (see
`1.4.1 <#Sec:GravityBHTreeUsing>`__).

During the tree walk, subroutine
``physics/Gravity/Gravity_bhNodeContrib`` adds contributions of tree
...
:math:`Gm/r=Gm \, \mathrm{erf}(\alpha r)/r + Gm \, \mathrm{erfc}(\alpha r)/r`
...
To include the effects of gravity in your |flashx| executable, include
the option

   -with-unit=physics/Gravity

on your command line when you configure the code with ``setup``. The
default implementation is ``Constant``, which can be overridden by
including the entire path to the specific implementation in the command
line or ``Config`` file. The other available implementations are
``Gravity/GravityMain/Planepar``, ``Gravity/GravityMain/Pointmass`` and
``Gravity/GravityMain/Poisson``. The ``Gravity`` unit provides accessor
...
gravitational potential, additional storage is needed in ``unk`` for
...
variables may be needed. The variables ``GPOT_VAR`` and ``GPOT_VAR``,
...
this unit and setting the runtime parameter ``Gravity/useGravity`` true.
The constant of gravity can be set independently by runtime parameter
``Gravity/grv_bhNewton``; if it is not positive, the constant ``Newton``
from the |flashx| ``PhysicalConstants`` database is used. If parameters
``Grid/gr_bhPhysMACTW`` or ``Grid/gr_bhPhysMACComm`` are set, the
gravity unit MAC is used and it can be chosen by setting
``Gravity/grv_bhMAC`` to either ``ApproxPartialErr`` or
...
with the runtime parameter ``Gravity/grv_bhAccErr``. It has either the
...
is used if ``Gravity/grv_bhUseRelAccErr`` is set to True, and in this
...
``Gravity/grav_boundary_type`` and they can be ``isolated``,
``periodic`` or ``mixed``. In the case of mixed boundary conditions,
runtime parameters ``Gravity/grav_boundary_type_x``,
``Gravity/grav_boundary_type_y`` and ``Gravity/grav_boundary_type_z``
specify along which coordinate boundary conditions are periodic and
isolated (possible values are ``periodic`` or ``isolated``). Arbitrary
...
If runtime parameter ``Gravity/grav_boundary_type`` is ``periodic`` or
``mixed``, then the Ewald field for appropriate symmetry is calculated
at the beginning of the simulation. Parameter
``Gravity/grv_bhEwaldSeriesN`` controls the range of indices
:math:`i_1,i_2,i_3` in (eq. `[ewald_sum] <#ewald_sum>`__). There are two
...
:math:`1/r` term and its partial derivatives on a single cubic grid, and
the Ewald field is then approximated by the first order Taylor formula.
Parameter ``Gravity/grv_bhEwaldNPer`` controls number of grid points in
the :math:`x` direction in the case of ``periodic`` or in periodic
direction(s) in the case of ``mixed`` boundary conditions. Since an
elongated computational domain is often desired when
``Gravity/grav_boundary_type`` is ``mixed``, the cubic grid would lead
...
by adding ``bhtreeEwaldV42=1`` on the setup command line. The Ewald
...
``Gravity/grv_bhEwaldNRefV42``. If ``Gravity/grv_bhEwaldNRefV42`` is too
low to cover origin (where is the Ewald field discontinuous), then the
run is terminated. Each grid is composed of
``Gravity/grv_bhEwaldFieldNxV42`` :math:`\times`
``Gravity/grv_bhEwaldFieldNyV42`` :math:`\times`
``Gravity/grv_bhEwaldFieldNzV42`` points. When evaluation of the Ewald
...
``Gravity/grv_bhLinearInterpolOnlyV42`` (option ``true`` corresponds to
...
``Gravity/grv_useExternalPotential`` true. The parameter
``Gravity/grv_bhExtrnPotFile`` gives the name of the file with the
external potential and ``Gravity/grv_bhExtrnPotType`` specifies the
field symmetry: ``spherical`` for the spherical symmetry and ``planez``
for the planar symmetry with field being a function of the z-coordinate.
Parameters ``Gravity/grv_bhExtrnPotCenterY``,
``Gravity/grv_bhExtrnPotCenterX`` and ``Gravity/grv_bhExtrnPotCenterZ``
...
.. container:: center
   ...
   calculation.
      ...
      +==================+=========+==================+==================+
      | ``Gravit         | real    | -1.0             | constant of      |
      | y/grv_bhNewton`` |         |                  | gravity; if      |
      |                  |         |                  | :math:`<` 0, it  |
      |                  |         |                  | is obtained      |
      +------------------+---------+------------------+------------------+
      |                  |         |                  | from internal    |
      ...
      |                  |         |                  | database         |
      +------------------+---------+------------------+------------------+
      | ``Gra            | string  | "A               | MAC, other       |
      | vity/grv_bhMAC`` |         | pproxPartialErr" | option:          |
      |                  |         |                  | "MaxPartialErr"  |
      +------------------+---------+------------------+------------------+
      | ``Gravity/       | integer | 2                | degree of        |
      | grv_bhMPDegree`` |         |                  | multipole in     |
      |                  |         |                  | error estimate   |
      |                  |         |                  | in APE MAC       |
      +------------------+---------+------------------+------------------+
      | ``Gravity/grv_   | logical | .false.          | if .true.,       |
      | bhUseRelAccErr`` |         |                  | grv_bhAccErr has |
      |                  |         |                  | meaning of       |
      +------------------+---------+------------------+------------------+
      |                  |         |                  | relative error,  |
      |                  |         |                  | otherwise        |
      |                  |         |                  | absolute         |
      +------------------+---------+------------------+------------------+
      | ``Gravit         | real    | 0.1              | maximum allowed  |
      | y/grv_bhAccErr`` |         |                  | error in         |
      |                  |         |                  | gravitational    |
      +------------------+---------+------------------+------------------+
      |                  |         |                  | acceleration     |
      +------------------+---------+------------------+------------------+
...
.. container:: center
   ...
   conditions.
      ...
      | Variable         | Type    | Default          | Description      |
      +==================+=========+==================+==================+
      | ``Gravity/grav   | string  | "isolated"       | or "periodic" or |
      | _boundary_type`` |         |                  | "mixed"          |
      +------------------+---------+------------------+------------------+
      | ``Gravity/grav_b | string  | "isolated"       | or "periodic"    |
      | oundary_type_x`` |         |                  |                  |
      +------------------+---------+------------------+------------------+
      | ``Gravity/grav_b | string  | "isolated"       | or "periodic"    |
      | oundary_type_y`` |         |                  |                  |
      ...
      | oundary_type_z`` |         |                  |                  |
      +------------------+---------+------------------+------------------+
      | ``Gra            | boolean | true             | whether Ewald    |
      | vity/grv_bhEwald |         |                  | field should be  |
      | AlwaysGenerate`` |         |                  | regenerated      |
      +------------------+---------+------------------+------------------+
      | ``Gravity/grv_   | integer | :math:`10`       | number of terms  |
      | bhEwaldSeriesN`` |         |                  | in the Ewald     |
      |                  |         |                  | series           |
      +------------------+---------+------------------+------------------+
      | ``Gravity/g      | integer | 32               | number of        |
      | rv_bhEwaldNPer`` |         |                  | points+1 of the  |
      |                  |         |                  | Taylor expansion |
      +------------------+---------+------------------+------------------+
      | ``Gravity/gr     | string  | "ewald_coeffs"   | file with        |
      | v_bhEwaldFName`` |         |                  | coefficients of  |
      |                  |         |                  | the Ewald field  |
      |                  |         |                  | Taylor expansion |
      +------------------+---------+------------------+------------------+
      | `                | integer | :math:`32`       | size of the      |
      | `Gravity/grv_bhE |         |                  | Ewald field grid |
      | waldFieldNxV42`` |         |                  | in x-direction   |
      +------------------+---------+------------------+------------------+
      | `                | integer | :math:`32`       | size of the      |
      | `Gravity/grv_bhE |         |                  | Ewald field grid |
      | waldFieldNyV42`` |         |                  | in y-direction   |
      +------------------+---------+------------------+------------------+
      | `                | integer | :math:`32`       | size of the      |
      | `Gravity/grv_bhE |         |                  | Ewald field grid |
      | waldFieldNzV42`` |         |                  | in z-direction   |
      +------------------+---------+------------------+------------------+
      | ``Gravity/grv_   | integer | -1               | number of        |
      | bhEwaldNRefV42`` |         |                  | refinement       |
      |                  |         |                  | levels (nested   |
      |                  |         |                  | grids) for the   |
      |                  |         |                  | Ewald            |
      +------------------+---------+------------------+------------------+
      ...
      |                  |         |                  | automatically    |
      +------------------+---------+------------------+------------------+
      | ``Gravi          | logical | .true.           | if .false.,      |
      | ty/grv_bhLinearI |         |                  | semi-quadratic   |
      | nterpolOnlyV42`` |         |                  | interpolation is |
      |                  |         |                  | used for         |
      +------------------+---------+------------------+------------------+
      |                  |         |                  | interpolation in |
      |                  |         |                  | the Ewald field  |
      +------------------+---------+------------------+------------------+
      | ``               | string  | "                | file with the    |
      | Gravity/grv_bhEw |         | ewald_field_acc" | Ewald field for  |
      | aldFNameAccV42`` |         |                  | acceleration     |
      +------------------+---------+------------------+------------------+
      | ``               | string  | "                | file with        |
      | Gravity/grv_bhEw |         | ewald_field_pot" | coefficients of  |
      | aldFNamePotV42`` |         |                  | the Ewald field  |
      |                  |         |                  | for potential    |
      +------------------+---------+------------------+------------------+
...
.. container:: center
   ...
   | Variable         | Type    | Default          | Description      |
   +==================+=========+==================+==================+
   | ``Grav           | logical | .false.          | whether to use   |
   | ity/grv_bhUseExt |         |                  | external field   |
   | ernalPotential`` |         |                  |                  |
   +------------------+---------+------------------+------------------+
   | ``Gra            | logical | .true.           | whether to use   |
   | vity/grv_bhUsePo |         |                  | gravitational    |
   | issonPotential`` |         |                  | field calculated |
   |                  |         |                  | by the           |
   +------------------+---------+------------------+------------------+
   |                  |         |                  | tree solver      |
   +------------------+---------+------------------+------------------+
   | ``Gravity/grv_   | string  | "externa         | file containing  |
   | bhExtrnPotFile`` |         | l_potential.dat" | the external     |
   |                  |         |                  | gravitational    |
   |                  |         |                  | field            |
   +------------------+---------+------------------+------------------+
   | ``Gravity/grv_   | string  | "planez"         | type of the      |
   | bhExtrnPotType`` |         |                  | external field:  |
   |                  |         |                  | planar or        |
   |                  |         |                  | spherical        |
   |                  |         |                  | symmetry         |
   +------------------+---------+------------------+------------------+
   | `                | real    | 0.0              | x-coordinate of  |
   | `Gravity/grv_bhE |         |                  | the center of    |
   | xtrnPotCenterX`` |         |                  | the external     |
   |                  |         |                  | field            |
   +------------------+---------+------------------+------------------+
   | `                | real    | 0.0              | y-coordinate of  |
   | `Gravity/grv_bhE |         |                  | the center of    |
   | xtrnPotCenterY`` |         |                  | the external     |
   |                  |         |                  | field            |
   +------------------+---------+------------------+------------------+
   | `                | real    | 0.0              | z-coordinate of  |
   | `Gravity/grv_bhE |         |                  | the center of    |
   | xtrnPotCenterZ`` |         |                  | the external     |
   |                  |         |                  | field            |
   +------------------+---------+------------------+------------------+
...
There are two unit tests for the gravity unit. ``Poisson3`` is
...
be quantified. The second test, ``Poisson3_active`` is a modification of
``Poisson3`` to test the mapping of particles in
``Grid/Grid_mapParticlesToMesh``. Some of the mesh density is
...
test is similar to the simulation ``PoisParticles`` discussed in .
``PoisParticles`` is based on the Huang-Greengard Poisson gravity test
described in .
