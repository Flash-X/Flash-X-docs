...
The \setup script%\index{setup@\code{setup}}, found in the Flash-X root
directory, provides the primary command-line interface to configuring
the Flash-X source code. It is important to remember that the Flash-X code
is not a single application, but a set of independent code units which
can be put together in various combinations to create a multitude of
different simulations. It is through the setup script that the user
controls how the various units are assembled.


The primary job of the \setup script is to
\begin{itemize}
\item traverse the Flash-X source tree and link necessary files for a given
application to the \code{object/} directory
...
% \begin{comment}
% 
% % if someone wanted this they would run ./setup -help
% 
% \begin{verbatim}
%  prompt> ./setup -help
%  Processing Shortcut file: /home/username/Flash-X3/bin/setup_shortcuts.txt
...
%             -append-parfiles=<filenames>
%             -fbs -nofbs -tau=<makefile>
% 
%    (Misc Options)
%             -makehide -noclobber -portable -help
% 
%    * For GNU compatibility, options may be prefixed by a double dash (--opt) as well
%      as well as a single dash (-opt)
...
%    gravPfft         +ug +nofbs --with-unit=physics/Gravity/GravityMain/Poisson/Pfft
%    gravMgrid        +pm3 --with-unit=physics/Gravity/GravityMain/Poisson/Multigrid
%    gravMpole        +pm3 --with-unit=physics/Gravity/GravityMain/Poisson/Multipole
%    grid             --unit=Grid
%    hdf5             IO=hdf5
%    io               --with-unit=IO
...
% The \code{source/Simulation/SimulationMain} directory contains all example
% problems that come with the Flash-X code, and is where users should create
% their own custom problems (see \chpref{Chp:Creating new problems} for more
% information on this topic).
% 
% A ``problem'' consists of a specification of the code units to be
% used, a  set of initial and boundary conditions, possibly additional
% physics (\eg, a subgrid model for star formation), and a set of
% adjustable runtime parameters. A subdirectory of \code{SimulationMain}
% that has the same name as the problem contains source code
% implementing initial conditions and, in a few cases, the boundary
% conditions and extra physics. It also contains a fragment Makefile and a
% configuration file
% \code{Config}, read by \setup, which contains the information about
% required units and runtime parameters.
...
The \setup script uses the contents of the problem directory and the site/OS type,
together with a \code{Units} file, 
to generate the \code{object/} directory,
which contains links to the appropriate source files and makefile fragments.
The \code{Units} file lists the names of all units which need to be
included while building the Flash-X application. This file is automatically
generated when the user commonly provides the command-line \code{-auto} option, although it may be
assembled by hand. When \code{-auto} option is used, the \setup script
starts with the \code{Config} file of the problem specified, finds its
...
\begin{flashtip}
In \flashx, the \code{Units} file was located in the Flash-X root directory.
In \flashx, this file is found in the \code{object/} directory. 
\end{flashtip}
...
\begin{flashtip}[Reduce compilation time]
\begin{itemize}
%\item Use \code{gmake -j} to enable parallel builds in gmake.
\item To reuse compiled code when changing setup configurations, use the \code{-noclobber} setup option.
For details see \secref{Sec:ListSetupArgs}.
\end{itemize}
\end{flashtip}
