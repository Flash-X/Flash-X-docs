...
are written to the file \code{Simulation.h}.
...
% same text included in Simulation.h chapter
...
Although the \code{Simulation.h} file can distinguish between these two types of variables, many plotting routines cannot.
\end{flashtip} 
...
generated \code{Simulation.h}. Here \code{\metavar{SYM1}} is \metavar{sym1}
...
The preprocessor macros defined in \code{Simulation.h} for these arrays will have the
...
\code{Simulation.h} & contains simulation specific preprocessor macros, which change
based upon setup unlike \code{constants.h}. It is described in \chpref{Chp:Simulation.h} \eor

\hline %
\grayrow \api{Simulation/Simulation_mapIntToStr}\code{.F90} & contains code to map
an index described in \code{Simulation.h} to a string described in the \code{Config} file. \eor

\api{Simulation/Simulation_mapStrToInt}\code{.F90} & contains code to map a string
described in the \code{Config} file to an integer index described in the \code{Simulation.h}
file. \eor
